...
test('all locales link the same six sources with an initially unselected follow button',()=>{
 for(const lang of Object.keys(heatCopy)){const html=topicHeat(data,lang);assert.equal((html.match(/<li>/g)||[]).length,6);assert.match(html,/aria-pressed="false"/);assert.ok(html.includes(data.posts[0].url));}
});
