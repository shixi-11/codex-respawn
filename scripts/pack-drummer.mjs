...
const root=new URL('../',import.meta.url),count=24,columns=6;
...
const atlas=await sharp({create:{width:size*columns,height:size*4,channels:4,background:{r:0,g:0,b:0,alpha:0}}}).composite(cropped.map((input,i)=>({input,left:(i%columns)*size,top:Math.floor(i/columns)*size}))).webp({quality:86,alphaQuality:100,effort:6}).toBuffer();
await writeFile(new URL('public/assets/drummer-atlas.webp',root),atlas);
await writeFile(new URL('src/drummer-data.mjs',root),`// Fixed-camera Blender render, 24 frames at 30 fps.\nexport const drumFrames=${count},drumColumns=${columns},drumSize=${size},drumFps=30;\n`);
