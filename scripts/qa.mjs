...
await page.setViewportSize({width:1440,height:1000});await page.goto(base);await page.getByRole('button',{name:'Reset credits',exact:true}).click();assert.ok(await page.locator('.event-row:visible').evaluateAll(rows=>rows.every(r=>r.dataset.kind==='banked')));
...
await page.getByRole('button',{name:'Claude',exact:true}).click();assert.ok(await page.locator('.event-row:visible').evaluateAll(rows=>rows.length>0&&rows.every(row=>row.dataset.platformFeed==='claude')));await page.getByRole('button',{name:'Codex + Claude',exact:true}).click();
await page.locator('summary').first().click();assert.equal(await page.locator('details[open]').count(),1);await page.getByRole('combobox',{name:'Language'}).selectOption({label:'العربية'});await page.waitForURL('**/ar/');assert.equal(await page.locator('html').getAttribute('dir'),'rtl');
...
await page.goto(base);await page.screenshot({path:'.qa/desktop.png',fullPage:false});await writeFile('.qa/results.json',JSON.stringify({checks:checks.length,errors,interactions:'filters, pagination, FAQ, locale, event, keyboard'},null,2));console.log(`PASS: ${checks.length} language/viewport combinations, interaction flows and no page errors.`);
