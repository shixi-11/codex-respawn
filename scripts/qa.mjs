...
await page.getByRole('button',{name:'Claude',exact:true}).click();assert.ok(await page.locator('.event-row:visible').evaluateAll(rows=>rows.length>0&&rows.every(row=>row.dataset.platformFeed==='claude')));await page.getByRole('button',{name:'All',exact:true}).click();
