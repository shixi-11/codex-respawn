...
Optional human editorial work can use a locally authorized Grok subscription CLI, model `grok-4.6`, reasoning `high`, for current public Web/X research. A code or chronology review can use the operator's available Codex model at `medium` or `high` reasoning. This is an editorial choice, not a cron dependency. Keep OAuth, local profiles and subscription credentials off GitHub runners. Model output is advisory and must link to a primary source.
