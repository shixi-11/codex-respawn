...
export function clockPanel(platforms,lang,base,version,events,heat){
 ...
 <div class="clock-face"><div class="clock-status"><span>${e(t.next)}</span><span class="status-led" aria-hidden="true"></span></div><div class="clock-content"><div class="clock-reading"><p class="clock-verdict" data-clock-verdict>${e(s.state==='unknown'?w.waiting:t[s.state])}</p><div class="public-countdown" data-clock-countdown dir="ltr" ${s.state==='announced'?'':'hidden'}>${['--','--','--'].map((n,i)=>`<span class="time-unit"><strong data-digit="${i}">${n}</strong><small>${e([c.hours,c.minutes,c.seconds][i])}</small></span>${i<2?'<span class="clock-colon" aria-hidden="true">:</span>':''}`).join('')}</div><p class="public-reset-time" data-public-time></p></div><div class="station-art" aria-hidden="true"><img src="${base}assets/reset-station.webp" width="240" height="240" alt=""><span class="station-light"></span></div></div></div>
