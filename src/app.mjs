...
const filter=()=>{const matches=rows.filter(el=>(activePlatform==='all'||el.dataset.platformFeed===activePlatform));rows.forEach(el=>el.hidden=!matches.slice(0,limit).includes(el));const more=document.querySelector('.more');if(more)more.hidden=matches.length<=limit;const empty=document.querySelector('.empty');if(empty)empty.hidden=matches.length>0;};
document.querySelector('.more')?.addEventListener('click',()=>{limit+=5;filter();});filter();
