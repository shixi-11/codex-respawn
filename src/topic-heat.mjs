...
export function topicHeat(data,lang){
 ...
 return `<aside class="topic-heat" aria-label="${e(t[0])}"><div class="heat-summary">${drummer(lang)}<div><div class="heat-metric"><strong>${e(number)}</strong><span>${e(t[0])}</span></div><p class="heat-caption">${e(t[1])}</p></div></div><div class="heat-actions"><details class="heat-sources"><summary aria-label="${e(t[2])}" title="${e(t[2])}">ⓘ</summary><div class="heat-popover"><strong>${e(t[2])}</strong><p>${e(t[7])}</p><p>${e(t[8])} <time datetime="${e(data.observedAt)}">${e(date)}</time></p><ul>${data.posts.map(p=>`<li><a href="${e(p.url)}" target="_blank" rel="noopener noreferrer">@${e(p.url.split('/')[3])} · ${e(new Intl.NumberFormat(lang).format(p.views))} ↗</a></li>`).join('')}</ul></div></details><button type="button" class="heat-follow" data-heat-follow aria-pressed="false" aria-describedby="heat-feedback" title="${e(t[5])}" disabled><span data-heat-icon aria-hidden="true">♡</span><span data-heat-label>${e(t[3])}</span></button></div><p id="heat-feedback" class="heat-feedback" role="status" hidden></p></aside>`;
}
