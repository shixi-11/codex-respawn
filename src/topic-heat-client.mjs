...
if(button){
 ...
 button.addEventListener('click',()=>{try{const next=!selected;localStorage.setItem(key,next?'1':'0');selected=next;render();if(next)void drum.play();feedback.hidden=true;feedback.textContent='';}catch{feedback.textContent=t[6];feedback.hidden=false;}});
