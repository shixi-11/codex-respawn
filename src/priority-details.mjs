...
const rows={
en:['Latest relevant post','Previous resets','Earlier related posts','Completion announcement time; the exact reset time was not published.','No verified related post yet','Latest reset','Confirmed complete','Not a confirmed reset time'],
zh:['最新相关推文','前几次重置','更早的相关推文','显示完成公告的发布时间；原文未给出精确重置时刻。','暂无已核实的相关推文','上次重置','已确认完成','未确认重置时间'],
'zh-Hant':['最新相關推文','前幾次重置','更早的相關推文','顯示完成公告的發佈時間；原文未提供精確重置時刻。','暫無已核實的相關推文','上次重置','已確認完成','未確認重置時間'],
ja:['最新の関連投稿','過去のリセット','以前の関連投稿','完了発表の投稿時刻です。正確なリセット時刻は公表されていません。','確認済みの関連投稿はありません','直近のリセット','完了確認済み','リセット時刻は未確認'],
ko:['최신 관련 게시물','이전 초기화','이전 관련 게시물','완료 발표가 게시된 시간입니다. 정확한 초기화 시각은 공개되지 않았습니다.','확인된 관련 게시물이 없습니다','최근 초기화','완료 확인됨','초기화 시간 미확인'],
es:['Última publicación relacionada','Restablecimientos anteriores','Publicaciones anteriores','Hora del anuncio de finalización; no se publicó la hora exacta del restablecimiento.','Aún no hay publicaciones verificadas','Último restablecimiento','Finalización confirmada','Hora de restablecimiento sin confirmar'],
fr:['Dernière publication pertinente','Réinitialisations précédentes','Publications précédentes','Heure de l’annonce de fin ; l’heure exacte de la réinitialisation n’a pas été publiée.','Aucune publication vérifiée pour le moment','Dernière réinitialisation','Fin confirmée','Horaire de réinitialisation non confirmé'],
de:['Neuester relevanter Beitrag','Frühere Resets','Frühere Beiträge','Zeit der Abschlussmeldung; die genaue Reset-Uhrzeit wurde nicht veröffentlicht.','Noch kein bestätigter relevanter Beitrag','Letzter Reset','Abschluss bestätigt','Reset-Zeitpunkt nicht bestätigt'],
ar:['أحدث منشور ذي صلة','عمليات إعادة الضبط السابقة','منشورات سابقة ذات صلة','وقت نشر إعلان الاكتمال؛ لم يُنشر وقت إعادة الضبط الدقيق.','لا توجد منشورات موثّقة بعد','آخر إعادة ضبط','تأكد الاكتمال','موعد إعادة الضبط غير مؤكّد']
};
...
export function priorityDetails(platform,id,events,lang){
 ...
 const previous=list.filter(ev=>ev.kind==='global'&&ev.state==='reported'&&ev.id!==last?.id&&(!last||Date.parse(ev.publishedAt)<Date.parse(last.publishedAt))).slice(0,3);
 const when=at=>localResetTime(at,lang);
 return `<div class="latest-post"><div class="priority-label"><span>${e(t.latest)}</span>${latest?`<time datetime="${e(latest.publishedAt)}">${e(when(latest.publishedAt))}</time>`:''}</div>${latest?`<a class="tweet-source" href="${e(latest.sourceUrl)}" target="_blank" rel="noopener noreferrer"><span>@${e(latest.author)}</span><span class="source-action">${e(sourceAction[lang])}<span class="source-arrow" aria-hidden="true">↗</span></span></a><p class="tweet-excerpt" lang="en" dir="ltr">${e(latest.excerpt)}</p><p class="tweet-meaning"><span class="post-kind" data-kind="${e(latest.kind)}">${e(postKindLabel(latest.kind,lang))}</span> ${e(latest.kind==='global'&&latest.state==='reported'?t.complete:latest.state==='unconfirmed'?t.unconfirmed:postStateLabel(latest.state,lang))}</p>`:`<p>${e(t.empty)}</p>`}</div>
 <div class="last-reset"><span>${e(t.last)}</span><a data-last-reset href="${e(last?.sourceUrl||platform.profileUrl)}" target="_blank" rel="noopener noreferrer">${last?`<time datetime="${e(last.publishedAt)}" title="${e(last.publishedAt)}">${e(when(last.publishedAt))}</time> ↗`:e(watchCopy[lang].noHistory)}</a></div>${last?`<p class="reset-time-note">${e(t.timeNote)}</p>`:''}
 ${previous.length?`<details class="reset-history" open><summary>${e(t.previous)} <span aria-hidden="true">＋</span></summary><ol>${previous.map(ev=>`<li><a href="${e(ev.sourceUrl)}" target="_blank" rel="noopener noreferrer"><time datetime="${e(ev.publishedAt)}">${e(when(ev.publishedAt))}</time> ↗</a></li>`).join('')}</ol></details>`:''}`;
}
...
export function earlierPosts(events){const seen=new Set();return [...events].sort((a,b)=>Date.parse(b.publishedAt)-Date.parse(a.publishedAt)).filter(ev=>{const platform=belongs(ev,'claude')?'claude':'codex';if(!seen.has(platform)){seen.add(platform);return false;}return true;});}
