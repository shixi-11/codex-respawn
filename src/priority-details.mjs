...
export function priorityDetails(platform,id,events,lang){
 ...
 return `<div class="latest-post"><div class="priority-label"><span>${e(t.latest)}</span>${latest?`<time datetime="${e(latest.publishedAt)}">${e(when(latest.publishedAt))}</time>`:''}</div>${latest?`<a class="tweet-source" href="${e(latest.sourceUrl)}" target="_blank" rel="noopener noreferrer"><span>@${e(latest.author)}</span><span aria-hidden="true">↗</span></a><p class="tweet-excerpt" lang="en" dir="ltr">${e(latest.excerpt)}</p><p class="tweet-meaning"><span class="post-kind" data-kind="${e(latest.kind)}">${e(postKindLabel(latest.kind,lang))}</span> ${e(latest.kind==='global'&&latest.state==='reported'?t.complete:latest.state==='unconfirmed'?t.unconfirmed:postStateLabel(latest.state,lang))}</p>`:`<p>${e(t.empty)}</p>`}</div>
