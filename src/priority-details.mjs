...
export function priorityDetails(platform,id,events,lang){
 ...
 ${previous.length?`<details class="reset-history"><summary>${e(t.previous)} <span aria-hidden="true">＋</span></summary><ol>${previous.map(ev=>`<li><a href="${e(ev.sourceUrl)}" target="_blank" rel="noopener noreferrer"><time datetime="${e(ev.publishedAt)}">${e(when(ev.publishedAt))}</time> ↗</a></li>`).join('')}</ol></details>`:''}`;
}
